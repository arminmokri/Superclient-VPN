...
def parseLink(link):
    if link.startswith(ssscheme):
        ...
        print(
            "ERROR: This script supports only vmess://(N/NG) and vless://(N/NG) and ss:// links"
        )
        return None
    ...
    for _in in _ins:
        ...
        if _tplKey in TPL:
            ...
            if _proto == "dns":
                ...
                _c["outbounds"].append({"protocol": "dns", "tag": "dns-out"})
...
def fill_dns(_c):
    if option.localdns != "":
        dns = {"address": option.localdns, "port": 53, "domains": ["geosite:cn"]}
